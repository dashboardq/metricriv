...
class PlausibleVisitorsService {
        public static function visitors($req, $res) {
        ...
        $result = self::callVisitors($val['site'], $connection, $collection->data['user_id'], $range, $ago);
        if($result == -1) {
        ...
        $args['name'] = $val['name'];
        $args['status'] = 'initial';
        $args['method'] = json_encode(['app\services\trackings\plausible\PlausibleVisitorsService', 'visitorsUpdate']);
    ...
    public static function callVisitors($site, $connection, $user_id, $range, $ago) {
        ...
        $url = 'https://plausible.io/api/v1/stats/aggregate?site_id=' . urlencode($site) . '&period=custom&date=' . $dates['start'] . ',' . $dates['end'] . '&metrics=visitors';
        $result = $rest->get($url);
        ...
        $output = $result->results->visitors->value;

        return $output;
    }

    public static function visitorsUpdate($tracking, $manual_result = null) {
        if($manual_result) {
            $result = $manual_result;
        } else {
            $connection = Connection::find($tracking->data['connection_id']);
            $result = self::callVisitors($tracking->data['values']['site'], $connection, $tracking->data['collection']['user_id'], $tracking->data['values']['range'], $tracking->data['values']['ago']);
        }
