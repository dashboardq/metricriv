...
class PlausibleVisitDurationService {
        public static function visitDuration($req, $res) {
        ...
        $result = self::callVisitDuration($val['site'], $connection, $collection->data['user_id'], $range, $ago);
        if($result == -1) {
        ...
        $args['name'] = $val['name'];
        $args['status'] = 'initial';
        $args['method'] = json_encode(['app\services\trackings\plausible\PlausibleVisitDurationService', 'visitDurationUpdate']);
    ...
    public static function callVisitDuration($site, $connection, $user_id, $range, $ago) {
        ...
        $url = 'https://plausible.io/api/v1/stats/aggregate?site_id=' . urlencode($site) . '&period=custom&date=' . $dates['start'] . ',' . $dates['end'] . '&metrics=visit_duration';
        $result = $rest->get($url);

        if(isset($result->error)) {
            // The error does not branch out based on API Key or incorrect site. It is the same error returned by Plausible.
             return -1;
        } elseif(!isset($result->results->visit_duration->value)) {
             return -1;
        }

        $output = $result->results->visit_duration->value;

        return $output;
    }

    public static function visitDurationUpdate($tracking, $manual_result = null) {
        if($manual_result) {
            $result = $manual_result;
        } else {
            $connection = Connection::find($tracking->data['connection_id']);
            $result = self::callVisitDuration($tracking->data['values']['site'], $connection, $tracking->data['collection']['user_id'], $tracking->data['values']['range'], $tracking->data['values']['ago']);
        }
