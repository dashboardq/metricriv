...
class PlausiblePageViewsService {
        public static function pageViews($req, $res) {
        ...
        $result = self::callPageViews($val['site'], $connection, $collection->data['user_id'], $range, $ago);
        if($result == -1) {
        ...
        $args['name'] = $val['name'];
        $args['status'] = 'initial';
        $args['method'] = json_encode(['app\services\trackings\plausible\PlausiblePageViewsService', 'pageViewsUpdate']);
    ...
    public static function callPageViews($site, $connection, $user_id, $range, $ago) {
        ...
        $url = 'https://plausible.io/api/v1/stats/aggregate?site_id=' . urlencode($site) . '&period=custom&date=' . $dates['start'] . ',' . $dates['end'] . '&metrics=pageviews';
        $result = $rest->get($url);

        if(isset($result->error)) {
            // The error does not branch out based on API Key or incorrect site. It is the same error returned by Plausible.
             return -1;
        } elseif(!isset($result->results->pageviews->value)) {
             return -1;
        }

        $output = $result->results->pageviews->value;

        return $output;
    }

    public static function pageViewsUpdate($tracking, $manual_result = null) {
        if($manual_result) {
            $result = $manual_result;
        } else {
            $connection = Connection::find($tracking->data['connection_id']);
            $result = self::callPageViews($tracking->data['values']['site'], $connection, $tracking->data['collection']['user_id'], $tracking->data['values']['range'], $tracking->data['values']['ago']);
        }
