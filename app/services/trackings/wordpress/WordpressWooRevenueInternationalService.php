...
class WordpressWooRevenueInternationalService {
        public static function wooRevenueInternational($req, $res) {
        ...
        // Advanced connection
        $type = 'woo_revenue_international';
        ...
        $args['name'] = $val['name'];
        $args['status'] = 'initial';
        $args['method'] = json_encode(['app\services\trackings\wordpress\WordpressWooRevenueInternationalService', 'wooRevenueInternationalUpdate']);
    ...
    public static function parseBody($body) {
        if(isset($body->value)) {
            // TODO: Need to be able to handle other currency.
            $output = '$' . number_format($body->value);
            $output = ao()->hook('app_woo_revenue_international', $output);
            return $output;
        } else {
            return -1;
        }
    }
    public static function wooRevenueInternationalUpdate($tracking, $manual_result = null) {
        if($manual_result) {
