...
class WordpressPagesService {
    public static function pages($req, $res) {
        ...
        $url = WordpressService::parseURL(ao()->request->user_id, $api_key, 'wp_pages', $range, $ago);
        if($url === false) {
        ...
        $args['name'] = $val['name'];
        $args['status'] = 'initial';
        $args['method'] = json_encode(['app\services\trackings\wordpress\WordpressPagesService', 'pagesUpdate']);
    ...
    public static function pagesUpdate($tracking, $manual_result = null) {
        if($manual_result) {
