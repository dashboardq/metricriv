...
class WordpressPostsService {
    public static function posts($req, $res) {
        ...
        $url = WordpressService::parseURL(ao()->request->user_id, $api_key, 'wp_posts', $range, $ago);
        if($url === false) {
        ...
        $args['name'] = $val['name'];
        $args['status'] = 'initial';
        $args['method'] = json_encode(['app\services\trackings\wordpress\WordpressPostsService', 'postsUpdate']);
    ...
    public static function postsUpdate($tracking, $manual_result = null) {
        if($manual_result) {
